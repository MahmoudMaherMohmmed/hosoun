...
Route::middleware('auth')->group(function () {
    Route::post('myfatoorah', 'MyFatoorahsController@index')->name('myfatoorah');
    Route::get('myfatoorah/callback', 'MyFatoorahsController@callback')->name('myfatoorah.callback');

    Route::get('moyasar/callback', 'MoyasarController@callback')->name('moyasar.callback');
});
    ...
    Route::middleware(['IsInstalled', 'switch_languages', 'ip_block'])->group(function () {
        ...
        Route::middleware(['is_active', 'auth', 'maintanance_mode'])->group(function () {
            ...
            Route::middleware(['is_admin'])->group(function () {
                ...
                Route::post('/quickupdate/class/{id}', 'QuickUpdateController@classQuick')->name('class.quick');
                    ...
                    if (\DB::connection()->getDatabaseName() && Schema::hasTable('settings')) {
                        if (env('IS_INSTALLED') == 1) {
                            ...
                            Route::prefix('googlemeet')->group(function () {
                                ...
                                // Route::post('create/meeting','GoogleMeetController@store')->name('googlemeet.meeting.create1');
                            ...
                            Route::get('setting', 'GoogleMeetController@googlemeetsetting')->name('googlemeet.setting');
                            //Route::resource('gcalendar', 'GoogleMeetController');
        ...
        Route::middleware(['is_verified'])->group(function () {
            ...
            Route::get('store', 'HomeController@onlineStore')->name('online_store.show');
            ...
            Route::view('show/help', 'front.support')->name('help.show');

            Route::view('terms_condition', 'front.terms_condition');
            ...
            // custom routes
            Route::post('/quickupdate/course/{id}', 'QuickUpdateController@courseQuick')->name('course.quick');
            Route::post('/quickupdate/courseinclude/{id}', 'QuickUpdateController@includedQuick')->name('include.quick');
            Route::post('/quickupdate/what/{id}', 'QuickUpdateController@whatQuick')->name('what.quick');
            Route::post('/quickupdate/Chapter/{id}', 'QuickUpdateController@ChapterQuick')->name('Chapter.quick');
            Route::post('/quickupdate/class/{id}', 'QuickUpdateController@classQuick')->name('class.quick');
