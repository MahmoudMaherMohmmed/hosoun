                            ...
                            <ul class="btm-40">
                                @if($courseinclude->isNotEmpty())
                                    ...
                                    @foreach($course->include as $in)
                                        @if($in->status ==1)
                                            <li><i class="fa {{ $in->icon }}"></i>{!! str_limit(strip_tags($in->detail), $limit = 50, $end = '...') !!}</li>
                                        @endif
                                    @endforeach
                                @endif
                            </ul>
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-6 col-md-6">
                                <div class="product-learn-dtl">
                                    <ul>
                                        <li><i class="flaticon-tick-inside-circle"></i>{!! str_limit(strip_tags($wl['detail']), $limit = 120, $end = '...') !!}</li>
                                    </ul>
                                </div>
                            </div>
                            @endif
                        ...
                        <div class="col-lg-9 col-6"  >
                           @php
                                ...
                                function convertToHoursMins($time, $format = '%02d:%02d') {
                                    ...
                                    $hours =floor($time / 60);
                                    $minutes = ($time % 60);
                                    return $hours . __('frontstaticword.hours') . $minutes . __('frontstaticword.course_length');
                                ...
                                $courseDuration = convertToHoursMins($classtwo, '%02dh %02d');
                                // FSMS
                            @endphp

                            <small>{{ $chapterCount . __('frontstaticword.sections') . " • " .$classesCount . __('frontstaticword.lectures') . " • " . $courseDuration }}</small>

                        </div>

                        <div class="col-lg-3 col-6 col-xs-6 nopadding">
                            <button type="button" onclick="toggleAllSections()" class="btn btn-link courseToggle"><span style="color:#0384a3"><strong>{{__('frontstaticword.expand')}}</strong></span></button>
                            <button type="button" onclick="toggleAllSections()" class="btn btn-link courseToggle" style="display:none"><span style="color:#0384a3"><strong>{{__('frontstaticword.collapse')}}</strong></span></button>
                        </div>
                    </div>
                    <!-- FSMS -->
               ...
               @php
                ...
                <div class="learning-review btm-40">
                    ...
                    @php
                        ...
                        if(empty($orders)) {
                            $records = auth()->user()->orders->where('bundle_id', '>' , 0);
                            foreach($records as $record) {
                                $array = $record->bundle_course_id;
