					...
					<div class="dashboard-items">
						<ul>
							<li><i class="fa fa-bookmark"></i><a href="{{ route('mycourse.show') }}"
									title="Dashboard">{{ __('frontstaticword.MyCourses') }}</a></li>
							<li><i class="fa fa-heart"></i><a href="{{ route('wishlist.show') }}"
									title="Profile Update">{{ __('frontstaticword.MyWishlist') }}</a></li>
							<li><i class="fa fa-history"></i><a href="{{ route('purchase.show') }}" title="Followers">{{
									__('frontstaticword.PurchaseHistory') }}</a></li>
							<li><i class="fa fa-user"></i><a href="{{route('profile.show',Auth::User()->id)}}"
									title="Upload Items">{{ __('frontstaticword.UserProfile') }}</a></li>
							@if(Auth::User()->role == "user")
							<li><i class="fas fa-chalkboard-teacher"></i><a href="{{ route('beinstructor.show') }}"
									title="Become An Instructor">{{ __('frontstaticword.BecomeAnInstructor') }}</a></li>
							{{--<li><i class="fas fa-chalkboard-teacher"></i><a href="#" data-toggle="modal"
									data-target="#myModalinstructor" title="Become An Instructor">{{
									__('frontstaticword.BecomeAnInstructor') }}</a></li>--}}
							@endif
							<li><i class="fa fa-user"></i><a href="{{ url('bankdetail') }}" title="Upload Items">{{
									__('frontstaticword.MyBankDetails') }}</a></li>
						</ul>
					</div>
				</div>
				<div class="col-xl-9 col-lg-8">
					...
					<div class="profile-info-block">
							...
							<div class="col-lg-6">
								<div class="form-group">
									...
									<input type="text" id="name" name="fname" class="form-control"
										...
										value="{{ $orders->fname }}" required>
								</div>
								<div class="form-group">
									<label for="email">{{ __('frontstaticword.Email') }}</label>
									<input type="email" id="email" name="email" class="form-control"
										placeholder="info@example.com" required value="{{ $orders->email }}">
								</div>
							...
							<div class="col-lg-4">
								<div class="form-group">
									...
									<select id="country_id" class="form-control js-example-basic-single"
										...
										<option value="{{ $coun->country_id }}" {{ $orders->country_id ==
											$coun->country_id ? 'selected' : ''}}>{{ $coun->nicename }}
										</option>
										@endforeach
									</select>
								</div>
							...
							<div class="col-lg-4">
								<div class="form-group">
									...
									<select id="upload_id" class="form-control js-example-basic-single" name="state_id">
										...
										@foreach ($states as $s)
										<option value="{{ $s->id}}" {{ $orders->state_id==$s->id ? 'selected' : '' }}>{{
											$s->name}}</option>
										@endforeach
							...
							<div class="col-lg-4">
								<div class="form-group">
									...
									<select id="grand" class="form-control js-example-basic-single" name="city_id">
										...
										@foreach ($cities as $c)
										<option value="{{ $c->id }}" {{ $orders->city_id == $c->id ? 'selected' :
											''}}>{{ $c->name }}
										</option>
										@endforeach
									</select>
						...
						<div class="row">
							<div class="col-lg-4">
								<div class="form-group">
									<label for="age_group">{{ __('frontstaticword.AgeGroup') }}:</label>
									<select id="age_group"
										class="form-control {{ $errors->has('age_group') ? ' is-invalid' : '' }}"
										name="age_group" style="height: 4.2em">
										@if($orders->age_group==null)
										<option value="none" selected disabled hidden>
											{{ __('frontstaticword.AgeGroup') }}
										</option>
										@endif
										<option value="21-30" {{ $orders->age_group == '21-30' ? 'selected' : ''}}> 21 -
											30
										</option>
										<option value="31-40" {{ $orders->age_group == '31-40' ? 'selected' : ''}}> 31 -
											40
										</option>
										<option value="41-50" {{ $orders->age_group == '41-50' ? 'selected' : ''}}> 41 -
											50
										</option>
										<option value="51-60" {{ $orders->age_group == '51-60' ? 'selected' : ''}}> 51 -
											60
										</option>
										<option value="61" {{ $orders->age_group == '61' ? 'selected' : ''}}> {{
											__('frontstaticword.MoreThan') }} 60 </option>
									</select>
							...
							<div class="col-lg-4">
								<div class="form-group">
									<label for="academic_qualification">{{ __('frontstaticword.AcademicQualification')
										}}:</label>
									<select id="academic_qualification"
										class="form-control {{ $errors->has('academic_qualification') ? ' is-invalid' : '' }}"
										name="academic_qualification" style="height: 4.2em">
										@if($orders->academic_qualification==null)
										<option value="none" selected disabled hidden>
											{{ __('frontstaticword.AcademicQualification') }}
										</option>
										@endif

										<option value="0" {{ $orders->academic_qualification == '0' ? 'selected' : ''}}>
											{{
											__('frontstaticword.BA') }} </option>
										<option value="1" {{ $orders->academic_qualification == '1' ? 'selected' : ''}}>
											{{
											__('frontstaticword.Master') }} </option>
										<option value="2" {{ $orders->academic_qualification == '2' ? 'selected' : ''}}>
											{{
							...
							<div class="col-lg-4">
								<div class="form-group">
									<label for="academic_qualification">{{ __('frontstaticword.Employer') }}:</label>
									<input type="text"
										class="form-control{{ $errors->has('employer') ? ' is-invalid' : '' }}"
										name="employer"
										value="{{ $orders->employer!=null ? $orders->employer : old('employer') }}"
										id="employer" placeholder="{{ __('frontstaticword.Employer') }}">
						...
						<div class="password display-none" id="update-password">
							<div class="row">
								<div class="col-lg-6">
									<div class="form-group">
										<label for="confirmpassword">{{ __('frontstaticword.Password')
											}}:</label>
						...
						<div class="row">
							<div class="col-lg-6">
								<div class="social-block">
									<div class="form-group">
										<label for="facebook">{{ __('frontstaticword.FacebookUrl')
											}}</label><br>
										<div class="row">
											...
											<div class="col-lg-10 col-10">
												<input type="text" name="fb_url" value="{{ $orders->fb_url }}"
													id="facebook" class="form-control" placeholder="Facebook.com/">
											</div>
										</div>
									</div>
								</div>
								<div class="social-block">
									<div class="form-group">
										...
										<div class="row">
											...
											<div class="col-lg-10 col-10">
												<input type="text" name="youtube_url" value="{{ $orders->youtube_url }}"
													id="behance2" class="form-control" placeholder="youtube.com/">
											</div>
										</div>
									</div>
								</div>
							...
							<div class="col-lg-6">
											...
											<div class="col-lg-2 col-2">
												<div class="profile-update-icon">
													<div class="product-update-social-icons"><a
															...
															title="twitter"><i class="fab fa-twitter twitter"></i></a>
													</div>
												</div>
											</div>
											<div class="col-lg-10 col-10">
												<input type="text" name="twitter_url" value="{{ $orders->twitter_url }}"
													id="twitter" class="form-control" placeholder="Twitter.com/">
											</div>
										</div>
									</div>
								</div>
								<div class="social-block">
									<div class="form-group">
										<label for="dribbble2">{{ __('frontstaticword.LinkedInUrl')
											}}</label><br>
										<div class="row">
											<div class="col-lg-2 col-2">
												<div class="profile-update-icon">
					...
					<div class="upload-items text-right">
						<button type="submit" class="btn btn-primary" title="upload items">{{
							__('frontstaticword.UpdateProfile') }}</button>
					</div>
