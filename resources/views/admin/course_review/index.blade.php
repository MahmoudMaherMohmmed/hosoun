...
@section('title', __('adminstaticword.Course') . ' ' . __('adminstaticword.Review') . ' - ' . __('adminstaticword.Admin'))
@section('body')
                      ...
                      <tr>
                        ...
                        <td>
                          <form action="{{ route('course.quick',$cat->id) }}" method="POST">
                            {{ csrf_field() }}
                            <button  type="Submit" class="btn btn-xs table-status {{ $cat->featured ==1 ? 'btn-success' : 'btn-danger' }}">
                              @if($cat->featured ==1)
