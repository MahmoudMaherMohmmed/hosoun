...
@section('title', __('adminstaticword.DatabaseBackup') . ' - ' . __('adminstaticword.Admin'))
@section('body')
                ...
                <div class="panel-body">
                    <div class="well">
                        ...
                        <form action="{{ action('DatabaseController@update') }}" method="POST"
                            ...
                            <div class="input-group input-group-sm">
                                ...
                                <span class="input-group-btn">
                                    <button type="submit"
                                        class="btn btn-primary btn-block">{{ __('adminstaticword.Save') }}!</button>
                                </span>
                            </div>
                        ...
                        <div class="panel panel-default">
                            <div class="panel-body">
                                <form action="{{ action('DatabaseController@genrate') }}" method="POST"
                                    enctype="multipart/form-data">
                                    {{ csrf_field() }}
                                    {{ method_field('POST') }}
                                    ...
                                    <button type="submit"
                                        class="btn btn-default btn-block">{{ __('adminstaticword.GenerateBackup') }}</button>
                            ...
                            <div class="col-xs-12">
                                ...
                                <div class="list-backup">
                                    <ul>
                                        ...
                                        <li>
                                            Make sure <b>mysql dump is enabled on your server</b> for database backup
                                            and before run
                                            this or
                        ...
                        <div class="row">
                            ...
                            <div class="col-xs-12 col-md-6">
                                ...
                                @foreach (array_reverse($files) as $key=> $file)
                                    ...
                                    @else
                                    <li><a
                                            href="{{ URL::temporarySignedRoute('database.download', now()->addMinutes(1), ['filename' => basename($file)]) }}">{{ basename($file)  }}</a>
                                    </li>
                                    @endif
                                @endforeach
                            </div>
                            ...
                            <div class="col-xs-12 col-md-6">
                                ...
                                <form action="{{ action('DatabaseController@deletebackup') }}" method="POST"
                                    enctype="multipart/form-data">
                                    {{ csrf_field() }}
                                    {{ method_field('POST') }}
                                    <button type="submit"
                                        class="btn btn-default">{{ __('Delete All Backups') }}</button>
                                </form>
                            </div>
                        </div>
