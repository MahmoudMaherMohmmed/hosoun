...
@section('title', __('adminstaticword.Edit') . ' ' . __('adminstaticword.Country') . ' - ' . __('adminstaticword.Admin'))
@section("body")
        ...
        <div class="col-xs-12">
            <div class="box box-primary">
                ...
                <form id="demo-form2" class="inline-form clear-both" method="post" enctype="multipart/form-data"
                    action="{{url('admin/country/'.$countries->id)}}" data-parsley-validate
                    ...
                    <div class="box-body">
                        <div class="form-group row">
                            <label class="control-label col-md-3 col-sm-3 col-xs-12" for="country">
                                {{ __('adminstaticword.CountryName') }} <span
                                    class="redstar">*</span>
                            </label>

                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <select id="country" class="form-control js-example-basic-single" name="country">
                                    <option>{{ __('adminstaticword.ChooseCountry') }}:
                                    </option>
                                    @foreach ($allcountry as $c)
                                    <option value="{{ $c->id }}"
                                        {{ $countries->country_id == $c->id ? 'selected' : ''}}>
                                        {{ $c->nicename }}
                                    </option>
                                    @endforeach
                                </select>
