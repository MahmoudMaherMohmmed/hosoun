    ...
    <div class="box">
        ...
        <div class="box-header with-border">
            ...
            <div class="pull-right tools">
                <a title="{{ __('adminstaticword.Back') }}" href="{{ route('zoom.index') }}" class="btn btn-sm btn-default">
                    <i class="fa fa-reply"></i>
                </a>

                <a title="{{ __('adminstaticword.Edit') }} {{ __('adminstaticword.Meeting') }}" href="{{ route('zoom.edit',$response['id']) }}" class="btn btn-sm btn-success">
                    <i class='bx bx-edit'></i>
                </a>

                <a title="{{ __('adminstaticword.Delete') }} {{ __('adminstaticword.Meeting') }}" data-toggle="modal" data-target="#delete{{ $response['id'] }}"
        ...
        <div class="box-body">
            ...
            <hr>
            <h3>{{ __('adminstaticword.MeetingType') }}:@if($response['type'] == '2') {{ __('adminstaticword.ScheduledMeeting') }}
                @elseif($response['type'] == '3') Recurring Meeting with no fixed time @else Recurring Meeting with
                fixed time @endif</h3>
            <hr>
            <h3>{{ __('adminstaticword.MeetingTopic') }}: {{ $response['topic'] }}</h3>
            <hr>
            <h3>{{ __('adminstaticword.MeetingAgenda') }}:{{ isset($response['agenda']) ? $response['agenda'] : "" }}
            </h3>
            <hr>
            ...
            <hr>
            <h3>{{ __('adminstaticword.MeetingURL') }}: <a href="{{ isset($response['join_url']) }}">{{ $response['join_url'] }}</a></h3>
            <hr>
            ...
            <h5>
                <i class="fa fa-camera" aria-hidden="true"></i> Host Video :
                {{ isset($response['settings']['host_video']) == true ? __('adminstaticword.Enable') : __('adminstaticword.Disable') }}
            </h5>
            <h5>
                <i class="fa fa-group" aria-hidden="true"></i> Join before Host :
                {{ isset($response['settings']['join_before_host']) == true ?  __('adminstaticword.Yes')  :  __('adminstaticword.No') }}
            </h5>
            <h5>
                <i class="fa fa-group" aria-hidden="true"></i> Join before Host :
                {{ isset($response['settings']['join_before_host']) == true ? __('adminstaticword.Yes')  :  __('adminstaticword.No')  }}
            ...
            <h5>
                <i class="fa fa-group" aria-hidden="true"></i> Participant Video :
