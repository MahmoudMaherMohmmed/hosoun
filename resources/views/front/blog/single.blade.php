...
@section('content')
  ...
  <section class="block-sec blog-content">
    <div class="container">
      <div class="row">
        <div class="col-12">
          <img src="{{ asset('images/blog/' . $blog->image) }}" class="img-fluid mb-5" alt="{{ $blog->heading }}">

          <div class="d-flex align-items-center text-accent mb-5">
            {{ $blog->created_at != null ? $blog->created_at->format('M d Y') : '' }} {{ $blog->user != null ? '/' : '' }}
            {{ $blog->user != null ? $blog->user->fname . ' ' . $blog->user->lname : '/' }}
          </div>
