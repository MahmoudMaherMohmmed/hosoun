...
@section('content')
  ...
  @if ($book_sub_categories->isNotEmpty())
          ...
          @foreach ($book_sub_categories as $key => $book_sub_category)
            <div class="col-md-6 p-3">
              <section class="books-collapse">
                <button class="books-head" type="button" data-bs-toggle="collapse"
                  data-bs-target="#grade_{{ $key }}" aria-expanded="false"
                  ...
                  <div class="d-flex flex-column align-items-start text-start gap-2">
                    ...
                    @endphp
                    <span
                      class="text-accent-2 fw-normal fs-14">{{ \App\Book::whereIN('book_child_category_id', $child_categories_ids)->count() }}
                      {{ __('frontstaticword.Books') }}</span>
                  </div>
                          ...
                          @foreach ($book_sub_category->child_categories as $child_category)
                            <tr>
                              <td class="fw-medium p-3">
                                <a href="{{ route('browse.child.category.books', $child_category->id) }}"
                                  class="d-flex align-items-center gap-3">
                                  {{ $child_category->title }}
                                  <span class="ms-auto fs-14 fw-normal">{{ $child_category->books->count() }}
                                    {{ __('frontstaticword.Books') }}</span>
                                </a>
                              </td>
                            </tr>
  ...
  @else
    <section class="block-sec page-content">
      <div class="container">
        <div class="content-wrapper text-center  border">
          <div class="row">
